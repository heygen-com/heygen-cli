...
| **Async jobs** | `--wait` blocks with exponential backoff; `--timeout` sets max (default 20m). Available on `video create`, `video-agent create`, `video-translate create`, `lipsync create` only — poll other async jobs with the group's `get`. 429s and 5xx retry automatically. |
