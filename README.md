...
| **Async jobs** | `--wait` blocks with exponential backoff; `--timeout` sets max (default 20m). Available on `video create`, `video-agent create`, `video-translate create`, `lipsync create` only — poll anything else with whichever `get` reads the id the create returned. 429 and selected transient 5xx (500/502/503/504) retry automatically on retry-eligible requests. |
