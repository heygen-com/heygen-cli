...
func TestBuildInvocation_JsonBodyBase(t *testing.T) {
	spec := &Spec{
		Flags: []FlagSpec{
			{Name: "title", Type: "string", Source: "body", JSONName: "title"},
		},
	...
	inv, err := spec.BuildInvocation(cmd, nil, jsonBody)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if inv.Body["video"] == nil {
		t.Error("Body[video] should be preserved from --json-body")
	}
}

func TestBuildInvocation_FlagOverridesJsonBody(t *testing.T) {
	spec := &Spec{
		Flags: []FlagSpec{
			{Name: "title", Type: "string", Source: "body", JSONName: "title"},
		},
	...
	// Flag wins over --json-body
	if inv.Body["title"] != "From Flag" {
		t.Errorf("Body[title] = %v, want %q (flag should override)", inv.Body["title"], "From Flag")
	}
	// Non-overlapping fields from --json-body preserved
	if inv.Body["video"] == nil {
		t.Error("Body[video] should be preserved from --json-body")
	}
}

func TestBuildInvocation_PositionalArgOverridesJsonBody(t *testing.T) {
	spec := &Spec{
		Args: []ArgSpec{
			{Name: "prompt", Target: "body", Param: "prompt"},
		},
	...
	inv, err := spec.BuildInvocation(cmd, []string{"From Positional"}, jsonBody)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Positional arg wins over --json-body
	if inv.Body["prompt"] != "From Positional" {
		t.Errorf("Body[prompt] = %v, want %q", inv.Body["prompt"], "From Positional")
	}
	// Other fields from --json-body preserved
