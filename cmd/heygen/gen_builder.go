...
func buildGenCommand(spec *command.Spec, ctx *cmdContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if jsonBodyFlag != "" {
				parsed, err := readJSONBody(jsonBodyFlag)
				if err != nil {
					return err
				}
				jsonBody = parsed
			...
			inv, err := spec.BuildInvocation(cmd, args, jsonBody)
			if err != nil {
				return err
			}
	...
	// Add --json-body for commands with JSON request bodies
	if spec.BodyEncoding == "json" {
		cmd.Flags().StringVar(&jsonBodyFlag, "json-body", "",
			"Raw JSON request body (inline JSON, file path, or - for stdin)")
	}
...
// readJSONBody reads JSON from inline string, file path, or stdin ("-").
func readJSONBody(input string) (map[string]any, error) {
	var data []byte
	var err error
	...
	if err := json.Unmarshal(data, &result); err != nil {
		return nil, clierrors.NewUsage(fmt.Sprintf("invalid JSON in --json-body: %v", err))
	}
	return result, nil
}
