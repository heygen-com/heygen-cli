...
This file mirrors `CLAUDE.md`. Keep the two files in sync when updating repo-specific agent guidance.
...
codegen/examples/  Per-group example YAML files (mandatory)
```
...
### Codegen name overrides
The codegen derives command names from HTTP method + URL path (POST = create, GET with param = get, etc.). When two endpoints in the same group produce the same name (e.g., two POST endpoints both mapping to "create"), codegen fails with a naming conflict error. Resolve conflicts by adding an entry to `nameOverrides` in `codegen/grouper.go`:
```go
