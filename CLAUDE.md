...
### New list commands
When adding a list endpoint, verify: verb is `list` not `get` (codegen uses last path segment to decide), `Paginated` is set if the API supports `limit`/`token`, and add curated `--human` columns in `cmd/heygen/columns.go`.
