...
var VideoTranslateCreate = &command.Spec{
	...
	Flags: []command.FlagSpec{
		...
		},
		{
			...
			Type:       "bool",
			Default:    "false",
			Help:       "Deprecated and ignored: captions are always generated; whether to display them is a download-side choice.",
			Required:   false,
			Enum:       nil,
			Min:        nil,
			Max:        nil,
		...
		},
		{
			...
			Type:     "string",
			Default:  "",
			Help:     "Destination folder ID in the caller's workspace, for example one returned by POST /v3/folders. Omit, pass null, or pass an empty string to place the translation at the workspace root. The caller must have write access to the folder; an unknown or foreign folder is rejected before the translation is created.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
