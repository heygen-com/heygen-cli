...
var VideoAgentCreate = &command.Spec{
	...
	Flags: []command.FlagSpec{
		...
		},
		{
			...
			Type:     "string",
			Default:  "",
			Help:     "Brand glossary ID controlling how custom terms are pronounced in the generated video's narration (for example, saying 'HeyGen' as 'hey-jen'). Pronunciation is applied to the synthesized audio only, so caption and subtitle text still show the original script wording. Discover IDs via GET /v3/brand-glossaries.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
		...
		},
		{
			...
			Type:     "string",
			Default:  "",
			Help:     "Brand kit ID to apply brand colors, fonts, and logos to the generated video. In enterprise workspaces with a locked brand policy, only the workspace default brand kit is accepted; omit this field to apply the workspace default automatically.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
			Source:   "body",
			JSONName: "brand_kit_id",
		},
		{
			Name:     "callback-id",
			Type:     "string",
		...
		},
		{
			...
			Type:     "string-slice",
			Default:  "",
			Help:     "Email addresses to grant access to. Valid only with `visibility: selected`, and every address must belong to a member of the caller's workspace (max 100). May be omitted or empty, leaving the session owner-only until members are added.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
		...
		},
		{
			...
			Type:     "string",
			Default:  "generate",
			Help:     "Session mode. 'generate' produces one video (fire-and-forget). 'chat' enables multi-turn interaction — the agent may pause for decisions and accept follow-up messages.",
			Required: false,
		...
		},
		{
			...
			Type:     "string",
			Default:  "space",
			Help:     "Who can open the session. `private`: the owner only. `space`: everyone in the workspace can open and drive the session. `selected`: the owner plus the workspace members listed in `members`. Defaults to `space`. Applies to the session and its conversation; does not change existing sessions.",
			Required: false,
			Enum:     []string{"private", "space", "selected"},
			Min:      nil,
			Max:      nil,
			Source:   "body",
			JSONName: "visibility",
...
var VideoAgentSend = &command.Spec{
	...
	Flags: []command.FlagSpec{
		...
		},
		{
			...
			Type:     "string",
			Default:  "",
			Help:     "Brand kit ID to apply for this message. In enterprise workspaces with a locked brand policy, only the workspace default brand kit is accepted.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
		...
		},
		{
			...
			Type:     "string",
			Default:  "",
			Help:     "Text message to the agent. Required when edit_plan is omitted; may be omitted or empty when edit_plan is provided.",
			Required: false,
			Enum:     nil,
			Min:      nil,
			Max:      nil,
